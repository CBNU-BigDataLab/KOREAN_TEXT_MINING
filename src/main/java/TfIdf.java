...
public class TfIdf {
    ...
    public static <TERM> Map<TERM, Double> tf(Collection<TERM> document, TfType type) {
        ...
        Integer sizeOfWordsPerDocument = document.size();
        for (TERM term : document) {
            //term frequency of word = wordCount of word / words per document
            tf.put(term, (tf.getOrDefault(term, 0.0) + 1) / sizeOfWordsPerDocument);
        }
